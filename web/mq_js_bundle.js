...
// Event queue to prevent RefCell re-entrancy panics.
// Browser events (mouse, keyboard, focus) can fire synchronously
// during WebGL operations inside frame(), which re-enters the
// WASM event handler while the RefCell is still borrowed.
var _WasmInFrame = false;
var _EventQueue = [];
    ...
    env: {
        ...
        run_animation_loop: function (blocking) {
            canvas.onmousemove = function (event) {
                ...
                var mx = Math.floor(event.movementX);
                var my = Math.floor(event.movementY);
                _safeEvent(function () {
                    wasm_exports.mouse_move(x, y);
                    if (mx != 0 || my != 0) {
                        wasm_exports.raw_mouse_move(mx, my);
                    }
                });
            };
            ...
            canvas.addEventListener('wheel',
                function (event) {
                    ...
                    var dy = -event.deltaY;
                    _safeEvent(function () {
                        wasm_exports.mouse_wheel(dx, dy);
                    });
                });
            ...
            canvas.onkeydown = function (event) {
                ...
                _safeEvent(function () {
                    wasm_exports.key_down(sapp_key_code, modifiers, repeat);
            ...
            canvas.onkeypress = function (event) {
                ...
                if (chrome_only == false) {
                    ...
                    _safeEvent(function () {
                        wasm_exports.key_press(charCode);
                    });
                }
            };
            ...
            canvas.addEventListener("touchstart", function (event) {
                ...
                for (const touch of event.changedTouches) {
                    let rp = mouse_relative_position(touch.clientX, touch.clientY);
                    touches.push({ id: touch.identifier, x: rp.x, y: rp.y });
                }
                _safeEvent(function () {
                    for (var i = 0; i < touches.length; i++) {
                        wasm_exports.touch(SAPP_EVENTTYPE_TOUCHES_BEGAN, touches[i].id, touches[i].x, touches[i].y);
                    }
                });
            });
            canvas.addEventListener("touchend", function (event) {
                ...
                _safeEvent(function () {
                    for (var i = 0; i < touches.length; i++) {
                        wasm_exports.touch(SAPP_EVENTTYPE_TOUCHES_ENDED, touches[i].id, touches[i].x, touches[i].y);
                    }
                });
            });
            canvas.addEventListener("touchcancel", function (event) {
                ...
                _safeEvent(function () {
                    for (var i = 0; i < touches.length; i++) {
                        wasm_exports.touch(SAPP_EVENTTYPE_TOUCHES_CANCELED, touches[i].id, touches[i].x, touches[i].y);
                    }
                });
            });
            canvas.addEventListener("touchmove", function (event) {
                ...
                _safeEvent(function () {
                    for (var i = 0; i < touches.length; i++) {
                        wasm_exports.touch(SAPP_EVENTTYPE_TOUCHES_MOVED, touches[i].id, touches[i].x, touches[i].y);
                    }
                });
            });
